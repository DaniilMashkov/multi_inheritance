...
def get_set_delete(person):
    person.name
    person.address = '765321 Lermontova ul.'
...
def main():
    ...
    old = min(timeit.repeat(partial(get_set_delete, person), number=1))
    new = min(timeit.repeat(partial(get_set_delete, person_test), number=1))
